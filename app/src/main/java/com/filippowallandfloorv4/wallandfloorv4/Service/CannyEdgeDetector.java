...
public class CannyEdgeDetector {
    ...
    private void readLuminance() {
        ...
        if (type == Bitmap.Config.RGB_565 || type == Bitmap.Config.ARGB_8888){
            //int[] pixels = (int[]) sourceImage.getData().getDataElements(0, 0, width, height, null);
            int[] pixels = new int[sourceImage.getHeight()*sourceImage.getWidth()];
            sourceImage.getPixels(pixels,0,sourceImage.getWidth(),0,0,sourceImage.getWidth(),sourceImage.getHeight());
            for (int i = 0; i < picsize; i++) {
                int p = pixels[i];
                int r = (p & 0xff0000) >> 16;
                int g = (p & 0xff00) >> 8;
